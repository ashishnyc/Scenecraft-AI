...
async def check_consistency(
    ...
    *,
    ...
    if not settings.ANTHROPIC_API_KEY:
        logger.warning("ANTHROPIC_API_KEY not set — skipping consistency check for task %s", task_id)
        return None
    ...
    try:
        ...
        logger.error("LLM call failed for consistency check (task %s): %s", task_id, exc)
        return None
